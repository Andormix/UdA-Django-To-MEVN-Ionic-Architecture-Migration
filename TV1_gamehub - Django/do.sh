...
shell() {
    _requires web_run
    exec -w /code web_run /bin/bash
}

migrate() {
    _requires web_run
    exec -w /code web_run python manage.py migrate "$@"
}

makemigrations() {
    _requires web_run
    exec -w /code web_run python manage.py makemigrations "$@"
}

check() {
    _requires web_run
    exec -w /code web_run python manage.py check
}
